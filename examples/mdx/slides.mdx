import Test from './test-component';
export const testProp = 50;

# Hello World 👋

- one
- two
- three

Notes: These are presenter notes. This text will be automatically exported as `notes` from either your MDX file (e.g. `import {notes} from './my-file.mdx'`) or from the CLI MDX file alias (`import {notes} from 'spectacle-user-mdx`).

If you're editing this project directly, use the first approach. If you're using the CLI, use the second approach.

---

## Next

<Test height={testProp} />

Notes: here are some more notes.

---

# Write your Spectacle Presentations in Markdown

## And seamlessly use React Components

**How sweet is that**
**(super sweet)**

---

![datboi](https://pbs.twimg.com/media/CkjFUyTXEAEysBY.jpg)

---

###### Typography

# Heading 1

## Heading 2

### Heading 3

#### Heading 4

##### Heading 5

###### Heading 6

Standard Text

---

> Example Quote

---

```javascript
class SuperCoolComponent extends React.Component {
  render() {
    return <p>code slide works in markdown too whaaaaat</p>;
  }
}
```
